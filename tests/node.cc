...
TEST_F(NodeTest, TestInteractionNode)
{
  ...
  EXPECT_EQ(NULL, interaction_node->cond()); // Defaults to NULL
  ...
  interaction_node->set_cond(cond);
  EXPECT_EQ(interaction_node->cond(), cond);
...
TEST_F(NodeTest, TestRecurNode)
{
  ...
  EXPECT_EQ(node->type(), sesstype::ST_NODE_RECUR);
...
TEST_F(NodeTest, TestContinueNode)
{
  ...
  EXPECT_EQ(node->type(), sesstype::ST_NODE_CONTINUE);
  EXPECT_EQ(node->label(), "label");
  node->set_label("new label");
  EXPECT_EQ(node->label(), "new_label");
  delete node;
...
TEST_F(NodeTest, TestChoiceNode)
{
  ...
  EXPECT_EQ(node->type(), sesstype::ST_NODE_CHOICE);
  ...
  EXPECT_EQ(node2->type(), sesstype::ST_NODE_CHOICE);
...
{
  auto *rng = new sesstype::RngExpr("i", new ValExpr(1), new VarExpr("N"));
  auto *node = new sesstype::ForNode(rng);
  EXPECT_EQ(node->type(), sesstype::ST_NODE_FOR);
...
{
  auto *node = new sesstype::AllReduceNode();
  EXPECT_EQ(node->type(), sesstype::ST_NODE_ALLREDUCE);
  ...
  auto *node_withsig = new sesstype::AllReduceNode(msgsig);
  EXPECT_EQ(node_withsig->type(), sesstype::ST_NODE_ALLREDUCE);
...
TEST_F(NodeTest, TestOneofNode)
{
  ...
  role->add_param(new sesstype::RngExpr(
        new sesstype::ValExpr(2), new sesstype::ValExpr(10)));
  auto *role2 = new sesstype::Role("R2");
  role2->add_param(new sesstype::RngExpr(
        new sesstype::ValExpr(2), new sesstype::ValExpr(10)));
  auto *node = new sesstype::OneofNode(role, 0);
  EXPECT_EQ(node->type(), sesstype::ST_NODE_ONEOF);
...
TEST_F(NodeTest, TestIfNode)
{
  ...
  auto *node = new sesstype::IfNode(cond);
  EXPECT_EQ(node->type(), sesstype::ST_NODE_IF);
...
TEST_F(NodeTest, BasicUsage)
{
  ...
  EXPECT_EQ(interaction_node->type(), sesstype::ST_NODE_SENDRECV);
  ...
  EXPECT_EQ(block_node->type(), sesstype::ST_NODE_ROOT);
  ...
  EXPECT_EQ(block_node->child(0)->type(), sesstype::ST_NODE_SENDRECV);
  EXPECT_EQ(block_node->child(1)->type(), sesstype::ST_NODE_ROOT);
  ...
  EXPECT_EQ(recur_node->type(), sesstype::ST_NODE_RECUR);
